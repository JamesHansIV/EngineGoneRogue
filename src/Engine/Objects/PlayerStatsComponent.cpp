...
void PlayerStatsComponent::Draw(){
    ...
    GridComponent stats_grid(static_cast<int>(m_Stats.size()),2, 0, 80, kCellHeight, kCellWidth);
    for (const auto& stat : m_Stats) {
        ...
        if(stat.first == "Speed" || stat.first == "DodgeSpeed")
        {
            ...
            std::stringstream ss;
            ss << std::fixed << std::setprecision(2) << value; // Set precision to 2
            box2 = new BoxWithText(ss.str(), src_rect, kBoxColor, false);
        } else {
            box2 = new BoxWithText(std::to_string(std::get<int>(stat.second)), src_rect, kBoxColor, false);
        }
...
void PlayerStatsComponent::FillStatsVector() {
    ...
    m_Stats.push_back(std::make_pair("DodgeSpeed", m_playerStats.GetDodgeSpeed()));
    m_Stats.push_back(std::make_pair("RangedDamage", m_playerStats.GetRangedDamage()));
    m_Stats.push_back(std::make_pair("MeleeDamage", m_playerStats.GetMeleeDamage()));
    m_Stats.push_back(std::make_pair("Experience", m_playerStats.GetExperience()));
    m_Stats.push_back(std::make_pair("Piercing", m_playerStats.GetPiercing()));
    m_Stats.push_back(std::make_pair("ArmorPercentage", m_playerStats.GetArmorPercentage()));
    m_Stats.push_back(std::make_pair("HPRegenRate", m_playerStats.GetHPRegenRate()));
    m_Stats.push_back(std::make_pair("Level", m_playerStats.GetLevel()));
    m_Stats.push_back(std::make_pair("LifeStealPercentage", m_playerStats.GetLifeStealPercentage()));
}