...
DestructibleItem::DestructibleItem(Properties& props) : Collider(props) {
    ...
    m_animation->AddAnimation(
        "item_idle",
        {"destructible_objects", {0, 0, 32, 32}, 1, 0, SDL_FLIP_NONE, false});
    m_animation->AddAnimation(
        "item_destroying",
        {"destructible_objects", {0, 0, 32, 32}, 7, 150, SDL_FLIP_NONE, false});
    m_animation->AddAnimation(
        "item_destroyed",
        {"destructible_objects", {0, 6, 32, 32}, 1, 0, SDL_FLIP_NONE, false});
...
void DestructibleItem::Update(float dt) {
    m_animation->Update();
    if (m_animation->GetAnimationID() == "item_destroying" &&
        m_animation->Ended()) {
        m_animation->SelectAnimation("item_destroyed");
    }
    if (m_animation->GetAnimationID() == "item_destroyed" &&
        m_animation->Ended()) {
...
GameObject* DestructibleItem::Copy() {
    return new DestructibleItem(this);
}
