    ...
    for (int i = 0; i < m_Colliders.size(); i++) {
        for (int j = i + 1; j < m_Colliders.size(); j++) {
            ...
            if (!r1->Velocity().IsZero() && CheckCollision(nextPos1, c2->GetDstRect())) {
                SDL_Log("c1 velocity (%f, %f)", r1->Velocity().X, r1->Velocity().Y);
                SDL_Log("calling oncollide from c1 (%f, %f, %d, %d)", nextPos1.x, nextPos1.y, nextPos1.w, nextPos1.h);
                c1->OnCollide(c2);
            }
            if (!r2->Velocity().IsZero() && CheckCollision(nextPos2, c1->GetDstRect())) {
                SDL_Log("c2 velocity (%f, %f)", r2->Position().X, r2->Position().Y);
                SDL_Log("calling oncollide from c2 (%f, %f, %d, %d)", nextPos2.x, nextPos2.y, nextPos2.w, nextPos2.h);
                c2->OnCollide(c1);
            }
        }
    }
