...
struct BoxWithText : public Box {
    BoxWithText(std::string text, const SDL_Rect& srcRect,
                const DrawColor& boxColor, bool fill, TextAlign alignment)
        : Box(srcRect, boxColor, fill),
          text(std::move(text)),
...
struct BoxWithTexture : public Box {
    BoxWithTexture(std::string textureID, const SDL_Rect& srcRect,
                   const DrawColor& boxColor, bool fill)
...
struct BoxWithCounter : public BoxWithTexture {
    BoxWithCounter(int count, const std::string& textureID,
                   const SDL_Rect& srcRect, const DrawColor& boxColor,
                   bool fill)
...
class GridComponent {
   public:
    explicit GridComponent(int rows, int columns, int startX, int startY,
                           int cellHeight, int cellWidth)
        : m_rows(rows),
          m_start_x(startX),
          m_start_y(startY),
          m_columns(columns),
          m_cell_height(cellHeight),
          m_cell_width(cellWidth) {
        InitializeGrid();
        ...
        for (int i = 0; i < m_rows; i++) {
            for (int j = 0; j < m_columns; j++) {
                ...
                if (box != nullptr) {
                    ...
                    SDL_Rect box_around = {box_x + renderer.GetCameraX(),
                                           box_y + renderer.GetCameraY(),
                                           m_column_widths[j], m_cell_height};
                    renderer.DrawRect(box_around, box->color, box->should_fill);

                    if (m_has_border) {
                        SDL_Rect box_border = {box_x + renderer.GetCameraX(),
                                               box_y + renderer.GetCameraY(),
                                               m_column_widths[j],
                                               m_cell_height};
                    ...
                    if (dynamic_cast<BoxWithTexture*>(box) != nullptr) {
                        DrawTextureOnBox(dynamic_cast<BoxWithTexture*>(box),
                                         box_x, box_y);
                    }
                    if (dynamic_cast<BoxWithText*>(box) != nullptr) {
                        DrawTextOnBox(dynamic_cast<BoxWithText*>(box), box_x,
                                      box_y);
                    }
                    if (dynamic_cast<BoxWithCounter*>(box) != nullptr) {
                        DrawCountOnBox(dynamic_cast<BoxWithCounter*>(box),
                                       box_x, box_y);
                    }
                }
            }
    ...
    void DrawTextureOnBox(BoxWithTexture* box, int xPos, int yPos) const {
        ...
        SDL_Rect dst_rect = {xPos + renderer.GetCameraX(),
                             yPos + renderer.GetCameraY(),
                             m_column_widths[box->col], m_cell_height};
        SDL_Rect src_rect = box->src_rect;
        renderer.Draw(box->TextureID, src_rect, dst_rect, 0.0F, nullptr,
                      SDL_FLIP_NONE);
    }
    ...
    void DrawCountOnBox(Box* box, int xPos, int yPos) const {
        ...
        int const count_x =
            xPos + renderer.GetCameraX() + m_column_widths[box->col] - 20;
        ...
        Texture* count_texture = renderer.AddTextTexture(
            "count", std::to_string(count), kCountColor);
        SDL_Rect src_rect = {0, 0, count_texture->GetWidth(),
                             count_texture->GetHeight()};
        int const count_texture_x =
            count_x + (count_width - count_texture->GetWidth()) / 2;
        int const count_texture_y =
            count_y + (count_height - count_texture->GetHeight()) / 2;
        SDL_Rect count_texture_rect = {count_texture_x, count_texture_y,
                                       count_texture->GetWidth(),
                                       count_texture->GetHeight()};
   ...
   private:
    ...
    void InitializeGrid() {
        m_grid = std::vector<std::vector<Box*>>(
            m_rows, std::vector<Box*>(m_columns, nullptr));
    }
};
