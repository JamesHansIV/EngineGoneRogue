...
void ChestDropModal::Draw() {
    ...
    GridComponent grid_container(1, 1, x, y - kCellHeight, kCellHeight,
                                 grid_width);
    Box* message_box =
        new BoxWithText("Chest Found! You found these items:", {0, 0, 0, 255},
                        kColor, true, TextAlign::KCenter);
    grid_container.AddBox(message_box, 0, 0);
    grid_container.DrawGrid();

    GridComponent box_container(static_cast<int>(m_items.size()), 3, x, y,
    ...
    for (ItemType const item_t : m_items) {
        ...
        Box* item_box =
            new BoxWithTexture(item.GetTextureID(), src_rect, kColor, false);
        Box* item_name = new BoxWithText(item.GetName(), src_rect, kColor, true,
                                         TextAlign::KCenter);
        Box* item_description = new BoxWithText(item.GetDescription(), src_rect,
                                                kColor, true, TextAlign::KLeft);
    ...
    int const chest_y =
        (window_height - grid_height) / 2 - kCellHeight - kChestTextureHeight;
    GridComponent chest_grid(1, 1, chest_x, chest_y, kChestTextureHeight,
                             kChestTextureWidth);  // Adjust the size to 100x100
    Box* chest_box = new BoxWithTexture("wooden-chest-idle", {1, 5, 18, 16},
                                        kTransparentColor, false);
